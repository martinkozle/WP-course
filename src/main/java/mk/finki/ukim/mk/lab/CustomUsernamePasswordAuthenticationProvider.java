...
public class CustomUsernamePasswordAuthenticationProvider implements AuthenticationProvider {
    ...
    public CustomUsernamePasswordAuthenticationProvider(UserService userService, PasswordEncoder passwordEncoder) {
        ...
        if (userService.findByUsername("admin").isEmpty()) {
            userService.register("admin", "admin", "admin", passwordEncoder.encode("admin"), null);
            this.userService.changeRole("admin", Role.ADMIN);
        }
        if (userService.findByUsername("user").isEmpty()) {
            userService.register("user", "user", "user", passwordEncoder.encode("user"), null);
        }
}
